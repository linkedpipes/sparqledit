...
export class SparqlLanguageServer {
    ...
    provideAutocompleteAdvice(query: string, cursorOffset: number): { concepts: OntologyConcept[], prefixProcessor: PrefixProcessor } {
        var autocompletitionContext = this.queryAdviceInferencer.providePropertyAutocompletion(query, cursorOffset);
        if (autocompletitionContext == null) {
            return null;
        }

        var queryAdvice = autocompletitionContext.queryAdvice;
        if (queryAdvice == null || !(queryAdvice instanceof PropertyQueryAdvice)) {
            return null;
        }
        
        return {
            concepts: queryAdvice.orderConcepts,
            prefixProcessor: new PrefixProcessor(autocompletitionContext.prefixes)
        };
    }
}