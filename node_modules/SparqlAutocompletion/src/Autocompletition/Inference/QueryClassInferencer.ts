...
import { Node, Block, NodeType } from '../Query/Query';
...
export class QueryClassInferencer {
    ...
    private propertyNodeSetVariable(result: QueryClassInferencerResult, subjectOrObject: Node, domainOrRange: IOntologyClass) {
        if (subjectOrObject.type == NodeType.Variable && domainOrRange != null) {
            this.setVariableType(result, subjectOrObject.value, domainOrRange)
        }
    }
    
    private findOrCreateOntologyClassForIri(classIri: string) {
        var ontologyClass: IOntologyClass = this.ontologyHiearchy.ontologyAssertions.findClassByIri(classIri);
        ...
        for (var triple of block.triples) {
            if (triple.predicate.type == NodeType.Iri) {
                ...
                if (currentPropertyName != null) {
                    this.propertyNodeSetVariable(result, triple.subject, currentPropertyName.domain);
                    this.propertyNodeSetVariable(result, triple.object, currentPropertyName.range);
                }
            }
        }
    }
