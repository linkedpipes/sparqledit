...
export class QueryAdviceInferencer {
    ...
    constructor(
        ...
        this.tracker = createParentOntologyTrackerFromSettings(parentOntologyClassTracking);
    ...
    private getAdvicesForVariable(
        ...
        if (inferenceVariable.isRdfTypeObject) {
            ...
            return new RdfTypeQueryAdvice(allAtomicClasses, prefixes);
        }
    ...
    providePropertyAutocompletion(queryContent: string, cursorOffset: number): { queryAdvice: IQueryAdvice, prefixes: StringDictionary<string> } {
        try {
            var queryWithCursor = injectCursor(queryContent, cursorOffset);
            var parser = new ERSParser();
            var query = parser.parse(queryWithCursor).query;
            return {
                queryAdvice: this.inferVariableAdvices(query, "cursor"),
