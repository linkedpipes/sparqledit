...
export class SparqlSignatureHelpProvider {
    private ontologyInferencer: QueryAdviceInferencer;

    constructor(private ontologyHiearchy: OntologyHiearchy) {
        this.ontologyInferencer = new QueryAdviceInferencer(this.ontologyHiearchy);
    }
    ...
    provideSignatureHelp(query: string, cursorOffset: number): SparqlSignatureHelp {
        try {
            var cursorContext = this.ontologyInferencer.inferVariableAdvices(this.queryWithCursorParserResult(query, cursorOffset).query, 'cursor');
        } catch (e) {
            return null;
        }
        ...
        console.log(cursorContext.possibleClasses);
        return {
