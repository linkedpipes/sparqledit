import { ListClass, IOntologyClass, AtomicClass, UnknownClass, UnionClass, IntersectionClass, RestrictionClass, ComplementClass } from "../../Ontology/OntologyClass";
...
export class OntologyClassSerializer extends BaseTypeSerializer<IOntologyClass> {
    serialize(serializeObject: IOntologyClass, serializationContext: SerializationContext) {
        ...
        else if (serializeObject instanceof ComplementClass) {
            result = {
                type: 5,
                cclass: this.serialize(serializeObject.value, serializationContext)
            };
        }
        else {
            throw new Error("Unknown class type to serialization.");
        }

        if (serializeObject.label.containsAnyLabel()) {
            var ontologyLabelSerializer = serializationContext.getTypeSerializer("OntologyLabel");
            result.label = ontologyLabelSerializer.serializeCommon(serializeObject.label, serializationContext);
        }

        return result;
    }

    deserializeClass(payload: any, serializationContext: SerializationContext): IOntologyClass {
        switch (payload.type) {
            case 0:
            ...
            case 5:                
                return new ComplementClass(this.deserialize(payload.cclass, serializationContext));
            default:
    ...
    deserialize(payload: any, serializationContext: SerializationContext): IOntologyClass {
        var deserializedClass = this.deserializeClass(payload, serializationContext);
        if (payload.label !== undefined) {
            var ontologyLabelSerializer = serializationContext.getTypeSerializer("OntologyLabel");
            deserializedClass.label = ontologyLabelSerializer.deserializeCommon(payload.label, serializationContext);
        }
