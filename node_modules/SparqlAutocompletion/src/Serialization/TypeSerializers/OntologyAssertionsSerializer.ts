...
export class OntologyAssertionsSerializer extends BaseTypeSerializer<OntologyAssertions> {
    ...
    buildDictionaries(ontologyAssertions: OntologyAssertions) {
        var classDictionary: { [key: string]: number } = {};
        ontologyAssertions.classes.forEach((ontologyClass, index) => classDictionary[ontologyClass.getText()] = index);
        ontologyAssertions.classDictionary = classDictionary;

        var subClassEdgesDictionary: { [superClass: number]: number[] } = [];
        ontologyAssertions.subClassEdges.forEach((edge: { superClass: number, subsetClass: number }) => {
            if (subClassEdgesDictionary[edge.superClass] == undefined) {
                subClassEdgesDictionary[edge.superClass] = [edge.subsetClass];
            }
            else {
                subClassEdgesDictionary[edge.superClass].push(edge.subsetClass);
            }
        });
        ontologyAssertions.subClassEdgesDictionary = subClassEdgesDictionary;


        var propertiesDictionary: { [key: string]: OntologyProperty } = {};
        ontologyAssertions.properties.forEach(property => {
            ontologyAssertions.addPropertyToDomainDictionary(property);
            propertiesDictionary[property.name] = property;
        });
        ontologyAssertions.propertiesDictionary = propertiesDictionary;
    }
    ...
    deserialize(payload: any, serializationContext: SerializationContext): OntologyAssertions {
        ...
        this.buildDictionaries(ontologyAssertions);
