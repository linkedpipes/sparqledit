...
import { IOntologyClass, AtomicClass, UnknownClass, RestrictionClass, IntersectionClass, UnionClass, ComplementClass } from "./OntologyClass";
...
export class OntologyClassParser {
    ...
    public getClassType(classNode: IRDFNode): IOntologyClass {
        ...
        if (nodeType != "BlankNode") {
            var unknownClassMessage = "Class is not blank node or Iri and has value: " + classNode.nominalValue;
            return new UnknownClass(unknownClassMessage);
        }
        ...
        var triples = currentTriples.match(null, RdfIri.owlComplementOf, null);
        if (triples.containsAny()) {
            return this.parseComplement(currentTriples, triples);
        }

        var text = "Problem with node" + classNode.nominalValue
            + " . It does not contain any supported class definition. Triples predicates of the node: "
            + currentTriples.getTriples().map(x => x.predicate.nominalValue).join(", ");
        return new UnknownClass(text);
    }
    ...
    private parseIntersection(currentTriples: IGraph, propertyTriples: IGraph) {
        try {
            var intersectionNode = propertyTriples.getFirst().object.nominalValue;
            var nodeCollection = this.schemaGraph.getCollection("_:" + intersectionNode);
        }
        catch (ex) {
            return new UnknownClass("Get collection problem on node: " + intersectionNode);
        }
    ...
    private parseUnion(currentTriples: IGraph, propertyTriples: IGraph) {
        ...
        try {
            ...
            return new UnknownClass("Get collection problem on node: " + unionNode);
        }
    ...
    private parseComplement(currentTriples: IGraph, propertyTriples: IGraph) {
        try {
            var complementClassNode = propertyTriples.getFirst().object;
            var complementClass = this.getClassType(complementClassNode);
            return new ComplementClass(complementClass);
        }
        catch (ex) {
            // TODO: Log into logger
            return new UnknownClass("Parse complement class problem: " + complementClassNode);
        }
    }
}