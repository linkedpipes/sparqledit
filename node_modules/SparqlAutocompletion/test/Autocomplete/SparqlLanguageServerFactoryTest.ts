...
describe("SparqlLanguageServerFactory", () => {
    ...
    it("should create server from format with raw data.", () => {
        var testCaseData = {
            type: "raw",
            value: require("raw-loader!./testData/rawDataTest.ttl")
        }

        var result = runStringifyTestCase(testCaseData);
        chai.expect(result.ontologyHiearchy.concepts.length).greaterThan(1);
    });

    it("should create server from format with precomputed data.", () => {
        var testCaseData = {
            type: "precomputed",
            value: require("raw-loader!./testData/precomputedDataTest.json")
        }
