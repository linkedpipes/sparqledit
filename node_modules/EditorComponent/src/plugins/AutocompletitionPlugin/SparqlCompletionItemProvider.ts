import { AutocompleteProviderClass, OntologyHiearchy, OntologyAssertionsBuilder, OntologyHiearchyBuilder, TurtleGraphWrapper } from "AutocompleteProvider";
...
export class SparqlCompletionItemProvider implements monaco.languages.CompletionItemProvider {
    ...
    injectCursor(query: string, offset: number) {
        return this.takeWhileNotWhitespace(query.substring(0, offset)) + ' ?cursor ' + query.substring(offset, query.length);
    }
    ...
    provideCompletionItems(model: monaco.editor.IReadOnlyModel, position: monaco.Position, token: monaco.CancellationToken): monaco.languages.CompletionItem[] {
