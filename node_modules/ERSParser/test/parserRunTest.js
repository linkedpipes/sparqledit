...
var ERSParserDebuggable = require('../src/ERSParser').ERSParserDebuggable;
...
describe("Parser should", function () {
    ...
    it('should parse simple query with halting error 2.', function () {
        var parser = new ERSParser();
        var testQueryContent = 'select * where { ?a ?b " }';
        var parserResult = parser.parse(testQueryContent);
        var end = "";
    });
    ...
    describe("halting error", function () {
        it('should work with wrong terminal on begining of text.', function () {
            var parser = new ERSParserDebuggable();
            var testQueryContent = 'UNKONWN CONSTRUCT { ?a }';
            // var parserResult = parser.parse(testQueryContent);
            var end = '';
        });
        
        it('should work choose correct tracedError.', function () {
            var parser = new ERSParserDebuggable();
            var testQueryContent = 'CONSTRUCT { ?a }';
            var parserResult = parser.parse(testQueryContent);
            var end = '';
        });
    });
