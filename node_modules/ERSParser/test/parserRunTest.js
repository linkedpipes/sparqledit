...
describe("Parser should", function () {
    ...
    it('should parse simple query with errors.', function () {
        var parser = new ERSParser();
        var testQueryContent = 'select * where { ?a ?b ?c,wrong. }';
        var parserResult = parser.parse(testQueryContent);
        chai.expect(parserResult.errors.length).greaterThan(0);
        chai.expect(Object.keys(parserResult.errors[0])).deep.equal(["type", "yylocStart", "yylocEnd"]);
    });
});
