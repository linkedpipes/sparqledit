...
function runPreParser(query) {
    ...
    return preParser.run(lexer, parser.terminals_);
}
...
describe("PreParser should", function () {
    ...
    describe("correctly work with brackets", function () {
        it("test case 1", function () {
            var query = 'filter (( ';
            var result = runPreParser(query);
            chai.expect(result.fixedQuery).equal('filter (( ))');
        });
        ...
        it("test case 5", function () {
            var query = '(saf((asd)(afsd)()df(()sfnbwe)))';
            var result = runPreParser(query);
            chai.expect(result.fixedQuery).equal(query);
        });

        it("test case 6", function () {
            var query = '( ?a ?b ( having';
            var result = runPreParser(query);
            chai.expect(result.fixedQuery).equal('( ?a ?b ( ))having');
        });
    });
});
