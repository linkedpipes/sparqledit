...
function ERSParserError(message, hash) {
  ...
  this.stackSize = hash.stackSize;
  this.stackComplexity = hash.cstack.reduce(function (acc, x) { return acc + x; }, 0);
}

function chooseHaltingError(extraTracedParserErrors) {
  var extraTracedParserErrorsLength = extraTracedParserErrors.length;
  if (extraTracedParserError <= 0) {
    throw new Error("No halting error to choose.");
  }
  var choosenHaltingError = extraTracedParserErrors[0];
  for (var i = 1; i < extraTracedParserErrorsLength; i++) {
    var extraTracedParserError = extraTracedParserErrors[i];
    if (extraTracedParserError.stackComplexity >= choosenHaltingError.stackComplexity) {
      var choosenHaltingError = extraTracedParserError;
    }
  }
  return choosenHaltingError;
}
...
function BaseERSParser(prefixes, baseIRI, parserType) {
  ...
  parser.extraTracedParserErrors = [];

  parser.trace = function (message, hash) {
    this.traceExtraParserErrors(hash.loc, hash.reallyExpected, hash.token, hash.stackSize, hash.cstack);
    this.tracedParserErrors.push(new ERSParserError(message, hash));
  }

  parser.traceExtraParserErrors = function (loc, reallyExpected, token, stackSize, cstack) {
    this.extraTracedParserErrors.push(new ERSParserError("Extra", {
      loc: loc,
  ...
  parser.parse = function () {
    ...
    try {
      ...
      result.extraTracedParserErrors = parser.extraTracedParserErrors;
    }
    catch (e) {
      var lines = arguments[0].split(/\r?\n/gm);
      var chosenHaltingError = chooseHaltingError(parser.extraTracedParserErrors);
      if (chosenHaltingError === undefined) {
        throw e;
      }
      var chosenHaltingErrorLoc = chosenHaltingError.loc;
      result = {
        query: null,
        tracedParserErrors: parser.tracedParserErrors,
        extraTracedParserErrors: parser.extraTracedParserErrors,
        recoverableErrors: [],
        haltingError: {
          type: "ClassicHaltingError",
          options: {
            originalMessage: chosenHaltingError.originalMessage,
            expectedTokens: chosenHaltingError.expected,
            unexpectedToken: chosenHaltingError.problemToken
          },
          range: {
            first_line: chosenHaltingErrorLoc.last_line,
            first_column: chosenHaltingErrorLoc.last_column,
