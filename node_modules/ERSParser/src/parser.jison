%{
  ...
  */
  ...
  function resolveIRI(iri) {
    ...
    if (/^[a-z]+:/.test(iri))
      return iri;
    if (!Parser.base) {
        if(!isBaseStatementMissingLogged){
            logParserError("BaseStatementMissing", {line:0,column:1}, {line:0,column:2});
            isBaseStatementMissingLogged = true;
        }
        base = "http://example.org/wrongBase/";
 ...
 if (attr)
  ...
  var parserErrors = []; 
  var isBaseStatementMissingLogged = false;

  Parser.resetParserErrors = function(){
      parserErrors = []; 
      isBaseStatementMissingLogged = false;
  };
  ...
  function createLocVariable(variable, startPosition, endPosition){
      return { 
            type: "var",
            name:  toVar(variable),
            range: {
  ...
  function createLocIri(iri, startPosition, endPosition){
      return { 
            type: "iri",
            iri:  iri,
            range: {
...
STRING_LITERAL1_MALFORMED       "'"(([^\u0027\u005C\u000A\u000D])|{ECHAR})*\r?\n
STRING_LITERAL2_MALFORMED       "\""(([^\u0022\u005C\u000A\u000D])|{ECHAR})*\r?\n
...
{STRING_LITERAL1_MALFORMED}   return 'STRING_LITERAL1_MALFORMED'
{STRING_LITERAL2_MALFORMED}   return 'STRING_LITERAL2_MALFORMED'
{NIL}                    return 'NIL'
{ANON}                   return 'ANON'
<<EOF>>                  return 'EOF'
.                        return 'INVALID'
...
StartNeterminal
    ...
    {
        var result = { query:$1, recoverableErrors: parserErrors, haltingError:null }
        return result;
    } 
//    | QueryOrUpdateUnit error EOF 
    // Hack terminal "BREAKER" must be used in some rule, otherwise jison dont add it to symbol table.  
    | 'CURSOR' 'BREAKER' 'BREAKER' 
    ;
QueryOrUpdateUnit
    ...
    | error ( Query | Update )
    {
     logParserError('PrefixOrBase', getStart(@-0), getStart(@1)); 
      if (Parser.base)
        $2.base = Parser.base;
      Parser.base = base = basePath = baseRoot = '';
      $2.prefixes = Parser.prefixes;
...
BaseDecl
    ...
    | 'BASE' IRIREF error
    {
      logParserError('BaseAfter', getEnd(@2), getStart(@3));
      Parser.base = resolveIRI($2)
      base = basePath = baseRoot = '';
    }
    | 'BASE' error
    {
        logParserError('Base', getEnd(@1), getStart(@2));
        base = basePath = baseRoot = '';
    }
    ;
PrefixDecl
    ...
    | 'PREFIX' error
    {
        logParserError('Prefix', getEnd(@1), getStart(@2)); 
        if (!Parser.prefixes) Parser.prefixes = {};
    }
    | 'PREFIX' PNAME_NS IRIREF error
    {
      logParserError('PrefixAfter', getEnd(@3), getStart(@4)); 
      if (!Parser.prefixes) Parser.prefixes = {};
      $2 = $2.substr(0, $2.length - 1);
      $3 = resolveIRI($3);
      Parser.prefixes[$2] = $3;
    ...
    ;
SelectQuery
    : SelectClause DatasetClauseListError WhereClause SolutionModifier -> extend($1, groupDatasets($2), $3, $4)
    ;
SubSelect
    : SelectClause WhereClause SolutionModifier ValuesClause? -> extend({ type: 'query' }, $1, $2, $3, $4)
    ;
SelectClause
    : 'SELECT' ( 'DISTINCT' | 'REDUCED' )? SelectError -> extend({ queryType: 'SELECT', variables: $3 === '*' ? ['*'] : $3 }, $2 && ($1 = lowercase($2), $2 = {}, $2[$1] = true, $2))
    ;
SelectError
    : ( SelectClauseItem+ | '*' )
    | error 
    {
        logParserError('Select', getEnd(@-0), getStart(@1));
        $$ = '*';
    }
    ;
SelectClauseItem
    : VAR -> toVar($1)
    | '(' ErrorExpression 'AS' VAR ')' -> expression($2, { variable: toVar($4) })
    ;
ConstructQuery
    : ConstructQueryError ConstructTemplate DatasetClauseListError WhereClause SolutionModifier -> extend({ queryType: 'CONSTRUCT', template: $2 }, groupDatasets($3), $4, $5)
    | ConstructQueryError DatasetClause* 'WHERE' '{' TriplesTemplate? '}' SolutionModifier -> extend({ queryType: 'CONSTRUCT', template: $5 = ($5 ? $5.triples : []) }, groupDatasets($2), { where: [ { type: 'bgp', triples: appendAllTo([], $5) } ] }, $7)
    ;
ConstructQueryError
    : 'CONSTRUCT' 
    | 'CONSTRUCT' error
    {
        logParserError("AfterConstruct",getEnd(@1), getStart(@2));
        $$ = $1;
    }
    ;
DescribeQuery
    : 'DESCRIBE' DescribeError DatasetClauseListError WhereClause? SolutionModifier -> extend({ queryType: 'DESCRIBE', variables: $2 === '*' ? ['*'] : $2.map(toVar) }, groupDatasets($3), $4, $5)
    ;
DescribeError
    : ( (VAR | iri)+ | '*' )
    | error
    {
        logParserError("Describe",getEnd(@-0), getStart(@1));
        $$ = '*';
    }
    ;
AskQuery
    : 'ASK' DatasetClauseListError WhereClause SolutionModifier -> extend({ queryType: 'ASK' }, groupDatasets($2), $3, $4)
    ;
DatasetClause
    : 'FROM' 'NAMED'? iri -> { iri: $3, named: !!$2, error: false }
    | 'FROM' error  
    {
        logParserError("DatasetClause", getEnd(@1), getStart(@2));
        $$ = { error: true };
    }
    | 'FROM' 'NAMED' error 
    {
        logParserError("DatasetClause" ,getEnd(@2), getStart(@3));
        $$ = { error: true };
    }
    ;
DatasetClauseListError
    : DatasetClause*
    | error 
    {
        logParserError("DatasetClauseList",getEnd(@-0), getStart(@1));
        $$ = {};
    }
    ;
WhereClause
...
GroupGraphPatternSubTail
    ...
    | GraphPatternNotTriplesError -> null
    ;
TriplesBlock
...
GraphPatternNotTriplesError
    : 'OPTIONAL' error 
    {
        logParserError('OPTIONAL error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    | 'GRAPH' error 
    {
        logParserError('GRAPH error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    | 'SERVICE' error 
    {
        logParserError('SERVICE error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    | 'FILTER' error 
    {
        logParserError('filter error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    | 'BIND' error 
    {
        logParserError('BIND error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    | 'VALUES' error 
    {
        logParserError('VALUES error', getEnd(@1), getEnd(@2));
        $$ = null;
    }
    ;
...
GraphPatternNotTriples
    ...
    | 'BIND' '(' ErrorExpression 'AS' VAR ')' -> { type: 'bind', variable: toVar($5), expression: $3 }
    | ValuesClause
    ;
Constraint
...
FunctionCall
    ...
    | iri '(' 'DISTINCT'? ( ErrorExpression ',' )* ErrorExpression ')' -> { type: 'functionCall', function: $1, args: appendTo($4, $5), distinct: !!$3 }
    ;
ExpressionList  
    : NIL -> []
    | '(' ( ErrorExpression ',' )* ErrorExpression ')' -> appendTo($2, $3)
    ;
ConstructTemplate
    : '{' ConstructTriples? '}' -> $2
    ;
...
TriplesSameSubjectError
    ...
    | error { 
        $$ = [];
        logParserError("TriplesSameSubject", getEnd(@-0), getEnd(@1));
    }
    ;
TriplesSameSubject
    : VarOrTerm PropertyListNotEmpty -> $2.map(function (t) { return extend(triple($1), t); })
    | 'CURSOR' -> [{subject: "?cursor", predicate: "?predicate", object: "?object"}]
    ;
PropertyList
    : PropertyListNotEmpty?
    ;
PropertyListNotEmpty
    : VerbObjectList (VerbObjectListSeparator)* -> unionAll($1, $2);
    ;
VerbObjectListSeparator
...
VerbObjectListError
    ...
    { 
        $$ = [];
        logParserError("VerbObjectList", getEnd(@-0), getEnd(@1));
    }
    ;
VerbObjectList
    : Verb ObjectList -> objectListToTriples($1, $2)
    | 'CURSOR' -> [{predicate: "?cursor", object: "?object"}]
    ;
Verb
    : VAR -> createLocVariable($1, getStart(@1), getEnd(@1));
    | iri -> createLocIri($1, getStart(@1), getEnd(@1));
    | 'a' -> createLocIri(RDF_TYPE, getStart(@1), getEnd(@1)); 
    ;
ObjectList
    : GraphNode (GraphNodeWithSeparator)*  -> ([$1].concat($2)).filter(function(n) { return n != null; })
    ;
GraphNodeWithSeparator 
    : ',' GraphNodeError -> $2
    ;
GraphNodeError
    : GraphNode
    ...
    { 
        $$ = null;
        logParserError("GraphNode", getEnd(@-0), getEnd(@1));
    }
    ;
GraphNode
    : VarOrTerm -> { entity: $1, triples: [] } /* for consistency with TriplesNode */
    | 'CURSOR' -> { entity: '?cursor', triples: [] }
    ;
VarOrTerm
    : VAR -> createLocVariable($1, getStart(@1), getEnd(@1));
    | iri -> createLocIri($1, getStart(@1), getEnd(@1));
...
ErrorExpression
    ...
    | error
    {
        logParserError("Expression", getEnd(@-0), getStart(@1));
        $$ = "?error" 
    }
    ;
Expression
...
AdditiveExpressionTail
    ...
    | ( '+' | '-' ) error {
        logParserError("ExpressionPlusMinus", getEnd(@1), getStart(@2));
        $$ = [$1, "?error"]
    }
    ...
    | NumericLiteralPositive error 
    {
        logParserError("ExpressionPlusMinusLiteral", getEnd(@1), getStart(@2));
        $$ = ['+', createOperationTree($1, [])]
    }
    | NumericLiteralNegative error
    {
        logParserError("ExpressionPlusMinusLiteral", getEnd(@1), getStart(@2));
        $$ = ['-', createOperationTree($1.replace('-', ''), [])]
    }
    ;
...
MultiplicativeExpressionTail
    ...
    ;
UnaryExpression
    : '+'? PrimaryExpressionError -> $2
    | '!'  PrimaryExpressionError -> operation($1, [$2])
    | '-'  PrimaryExpressionError -> operation('UMINUS', [$2])
    | '+'  error
    {
        logParserError("ExpressionUnaryOperation", getEnd(@1), getStart(@2));
        $$ = "?error"
    }
    | '!'  error 
    {
    ...
    ;
PrimaryExpressionError
    : PrimaryExpression
    | PrimaryExpression error 
    {
        logParserError("PrimaryExpressionTail", getEnd(@1), getStart(@2));
        $$ = $1;
    } 
    ;
PrimaryExpression
...
BuiltInCall
    ...
    | FUNC_ARITY1 '(' ErrorExpression ')' -> operation(lowercase($1), [$3])
    | FUNC_ARITY2 '(' ErrorExpression ',' ErrorExpression ')' -> operation(lowercase($1), [$3, $5])
    | 'IF' '(' ErrorExpression ',' ErrorExpression ',' ErrorExpression ')' -> operation(lowercase($1), [$3, $5, $7])
    ...
    | 'BNODE' '(' ErrorExpression ')' -> operation($1, [$3])
    | 'NOT'? 'EXISTS' GroupGraphPattern -> operation($1 ? 'notexists' :'exists', [degroupSingle($3)])
    ;
Aggregate
    : 'COUNT' '(' 'DISTINCT'? ( '*' | ErrorExpression ) ')' -> expression($4, { type: 'aggregate', aggregation: lowercase($1), distinct: !!$3 })
    | FUNC_AGGREGATE '(' 'DISTINCT'? ErrorExpression ')' -> expression($4, { type: 'aggregate', aggregation: lowercase($1), distinct: !!$3 })
    | 'GROUP_CONCAT' '(' 'DISTINCT'? ErrorExpression GroupConcatSeparator? ')'  -> expression($4, { type: 'aggregate', aggregation: lowercase($1), distinct: !!$3, separator: $5 || ' ' })
    ;
...
| STRING_LITERAL1_MALFORMED 
    {   
        logParserError("MalformedString",getStart(@1),getEnd(@1)); 
        unescapeString($1, 1)
    } 
    | STRING_LITERAL2_MALFORMED
    {   
...
iri
    ...
    | PNAME_LN
    {
      ...
      if (!expansion) {
        logParserError('Prefix_PNAME_LN',getStart(@1),getEnd(@1))
        $$ = "wrong"
      }
      else{
    ...
    | PNAME_NS
    {
      ...
      if (!($1 in Parser.prefixes)) {
          logParserError('Prefix_PNAME_NS',getStart(@1),getEnd(@1))
        $$ = "wrong"
      }
      else{
